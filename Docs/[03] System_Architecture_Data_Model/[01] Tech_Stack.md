...
| Blockchain (read/index) | **HAF** (required for Milestone 1 foundation) | Hive Application Framework: PostgreSQL projection of chain data (fork-aware). **Milestone 1 acceptance:** app reads Hive account/records via HAF (`HAF_DATABASE_URL`, `packages/hive` reader, `/api/v1/hive/accounts/:name`). Local demos may use a HAF-compatible `hafd` DB (`hive_haf`); shared Greateck HAF is the production target. A temporary custom listener + `hive_records` may exist only as interim escrow scaffolding — it is **not** the M1 acceptance read path. |
