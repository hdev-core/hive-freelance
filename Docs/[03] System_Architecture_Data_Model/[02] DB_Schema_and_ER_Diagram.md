...
- `CHECK` constraints enforced at DB level, not just application level
...
| `avatar_url` | TEXT | | |
| `location` | TEXT | | |
| `hourly_rate` | NUMERIC(10,2) | CHECK > 0 | Freelancers only |
| `portfolio_url` | TEXT | | |
...
| `hourly_rate` | NUMERIC(10,2) | | NULL for fixed contracts |
...
| `escrow_id` | INT | | Hive native escrow operation ID |
...
| `order` | INT | NOT NULL | Sequence within the contract |
...
| `hive_tx_id` | TEXT | | Transaction ID of the escrow operation |
| `created_at` | TIMESTAMP | NOT NULL DEFAULT now() | |
| `updated_at` | TIMESTAMP | NOT NULL DEFAULT now() | |
...
| payment → hive_records | M:1 | via `hive_tx_id` |
