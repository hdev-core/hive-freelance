...
## Milestone 1 standard: HAF for reads

**Architecture card (08 Jul):** reading/indexing on-chain records uses **HAF** (a PostgreSQL projection of the chain), **instead of** a permanent hand-rolled block listener + cache table.
...
Hive Blockchain
       │
HAF (hived + sql_serializer → PostgreSQL)
       │
App queries accounts / ops with SQL
```

HAF automatically:

- Streams blockchain data into PostgreSQL
- Handles blockchain forks
- Maintains synchronization

Instead of relying forever on custom parsing into `hive_records`, developers query chain data with SQL via HAF.

### Interim only: custom listener

The repo may still ship `apps/listener` + `hive_records` while HAF is being wired. That path is **scaffolding**, not Milestone 1 acceptance. Closing M1 requires an app read of Hive account/records **via HAF**.

WAX builds/signs txs; HAF is for reads. Do not use `dhive`.

Work plan: `Docs/[04] For Ali/[4.3] Tasks/[4.3.1] Infra/[01] Milestone_1_Hive_Layer_Work_Plan.md`.
...
| HAF (Milestone 1) | Reads/indexes blockchain data into PostgreSQL for the app |
