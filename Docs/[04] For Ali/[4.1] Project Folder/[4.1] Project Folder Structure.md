...
### apps/listener — interim Hive block watcher

Background poller that writes `hive_records`. **Milestone 1 standard for reads is HAF**, not this listener. Keep only as interim scaffolding until HAF reads are wired (see `[4.3] Tasks`).
